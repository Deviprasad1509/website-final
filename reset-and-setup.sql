...
-- Run this in Supabase SQL Editor to reset everything and set up fresh

-- Step 1: Drop all existing tables and related objects (if they exist)
...
DELETE FROM storage.objects WHERE bucket_id IN ('covers', 'books');
DELETE FROM storage.buckets WHERE id IN ('covers', 'books');
...
-- Step 6: Create RLS Policies (FIXED VERSION)
...
-- Step 9: Create trigger function for user signup
...
-- SUCCESS MESSAGE
SELECT 'Database setup completed successfully! All tables, policies, and triggers are now in place.' as status;